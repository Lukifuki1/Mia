...
class PAE:
    ...
    def get_performance_advisory(self) -> Dict[str, Any]:
        """Get performance advisory information"""
        try:
            status = self.get_pae_status()
            
            # Get recent recommendations
            recent_recommendations = []
            for rec in self.recommendations[-5:]:  # Last 5 recommendations
            ...
            performance_summary = {
                ...
                "active_regressions": len(self.regressions),
                "recommendations_pending": status.get("active_recommendations", 0),
                "analysis_active": self.analysis_active
            }
            ...
            return {
                ...
                "advisory_timestamp": self._get_deterministic_time() if hasattr(self, "_get_deterministic_time") else 1640995200
            }
            
        except Exception as e:
            self.logger.error(f"Failed to get performance advisory: {e}")
            return {
                "error": str(e),
                "status": {},
                "recent_recommendations": [],

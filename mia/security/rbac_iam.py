...
class RBACEngine:
    ...
    def _initialize_system_roles(self):
        ...
        super_admin = Role(
            ...
            permissions=list(Permission),
            created_at=self._get_build_timestamp(),
            is_system_role=True
        )
        ...
        for user in self.users.values():
            if user.username == username and user.is_active:
                if self._verify_password(password, user.password_hash):
                    ...
                    user.last_login = self._get_build_timestamp()
                    user.session_token = session_id
                    
                    self.logger.info(f"Uporabnik avtenticiran: {username}")
                    return session_id
        
        self.logger.warning(f"Neuspešna avtentikacija: {username}")
        return None
    ...
    def check_permission(self, session_id: str, permission: Permission) -> bool:
        ...
        if self._get_build_timestamp() > session.expires_at:
    ...
    def _create_session(self, user_id: str) -> str:
        ...
        session = Session(
            ...
            user_id=user_id,
            created_at=self._get_build_timestamp(),
            expires_at=self._get_build_timestamp() + timedelta(hours=8),
    ...
    def _hash_password(self, password: str) -> str:
        """Hash gesla"""
        salt = "mia_enterprise_salt"  # V produkciji bi bil naključen

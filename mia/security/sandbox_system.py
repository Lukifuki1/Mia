...
class SandboxEnvironment:
    ...
    def create_sandbox(self):
        ...
        try:
            # Ustvari začasni direktorij
            self.temp_dir = self._get_temp_path().mkdtemp(prefix="mia_sandbox_")
    ...
    def _is_safe_value(self, value: Any) -> bool:
        """Preveri, če je vrednost varna za sandbox"""
        # Dovoli osnovne tipe
        if isinstance(value, (str, int, float, bool, list, dict, tuple)):
            return True
        
        # Prepreči funkcije in module
    ...
    def _cleanup_sandbox(self):
        ...
        if self.temp_dir and os.path.exists(self.temp_dir):
            try:
                ...
                self.logger.info(f"Sandbox počiščen: {self.temp_dir}")
            except Exception as e:
                self.logger.error(f"Napaka pri čiščenju sandbox: {e}")
    ...
    def release_from_quarantine(self, quarantine_id: str) -> Optional[str]:
        ...
        try:
            ...
            if quarantine_path.exists():
                ...
                # Izbriši iz karantene
                quarantine_path.unlink()
                
                self.logger.info(f"Vsebina sproščena iz karantene: {quarantine_id}")
                return content
            
            return None
            
        except Exception as e:
            self.logger.error(f"Napaka pri sproščanju iz karantene: {e}")
            return None
